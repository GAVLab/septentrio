...
bool Septentrio::OnInitialized()
{
	...
	cout<<"DJB\n";
	if(m_MissionReader.GetConfigurationParam("OUTPUT_RATE",outputRate)) //Be sure to set in mission file
	{
		//outputType=convertToInteger(temp);

	} else {
		MOOSTrace("OUTPUT_RATE not set in mission file - Warning Septentrio Default Rate of 1 set\n");
	}
	RequestLogRate(outputRate);


	if(m_MissionReader.GetConfigurationParam("RANGE_OUTPUT_RATE",RangeOutputRate)) //Be sure to set in mission file
	{

	} else {
		MOOSTrace("RANGE_OUTPUT_RATE not set in mission file - Warning Septentrio Default Rate of 1 set\n");
	}
	RequestRangeLogRate(RangeOutputRate);
	...
	if(m_MissionReader.GetConfigurationParam("MANUAL_ANTENNA_POS",manual_position)) //Be sure to set in mission file
	{
		
	} else {
		MOOSTrace("MANUAL_ANTENNA_POS not set in mission file - Warning Septentrio Default to false\n");
		manual_position = false;
	}	
	
	if (manual_position)
	{
		m_MissionReader.GetConfigurationParam("x1",x1);
		m_MissionReader.GetConfigurationParam("y1",y1);
		m_MissionReader.GetConfigurationParam("z1",z1);
		m_MissionReader.GetConfigurationParam("x2",x2);
		m_MissionReader.GetConfigurationParam("y2",y2);
		m_MissionReader.GetConfigurationParam("z2",z2);

		cout<<"manual position true\t"<<x2<<endl;

		galcmd="gal\r\n";
		SendString(galcmd)==galcmd.length();
...
bool septentrio::RequestLog()
{
	...
	//string cmd="SetSBFOutput " +septentrioCom+ ", PVTCar+AttEuler \r\n";
	ConfigureLogging(sLogs);
	cout<< sLogs <<"\r\n";
	string cmd="SetSBFOutput " +septentrioCom+", "+ sLogs +"\r\n";
	return (SendString(cmd)==cmd.length());
...
void septentrio::update_ephemeris(double &blockTime){
	...
	string VarName="zEphem"+stringUtils::to_string((unsigned int)latest_ephemeris.PRN);
...
*/		
	...
	if (ant_num==1)
	{	//SetAntennaLocation manual 1 -0.46355 1.18067 0 <CR>
		cmd="SetAntennaLocation manual 1 "+x+" "+y+" "+z+"\r\n";
		return (SendString(cmd)==cmd.length());
	}
	else if (ant_num==2)
	{	//SetAntennaLocation manual 2 0.46355 1.18067 0 <CR>
